...
class MainFrame:
    ...
    def __init__(self, x_min=0, x_max=0, y_min=0, y_max=0, canvas_size=500):
        ...
        self.displayer = Canvas(self.canvas_frame, width=canvas_size, height=canvas_size, bg="white")
        self.displayer.pack()
    ...
    def add_axis(self):
        ...
        self.y_axis = self.displayer.create_line(y_position, self.canvas_size, y_position, 0,
                                       width=1, arrow=LAST)
        self.x_axis = self.displayer.create_line(0, x_position, self.canvas_size, x_position,
                                       width=1, arrow=LAST)
        ...
        for i in range(self.canvas_size + 1):
            if i % (self.canvas_size / 10) == 0:
                ...
                self.displayer.create_line(k + self.canvas_size // 2, -2 + x_position,
                                 k + self.canvas_size // 2, 2 + x_position,
                                 width=0.25, fill='black')

                self.displayer.create_text(k + self.canvas_size // 2, -10 + x_position,
        ...
        for j in range(self.canvas_size + 1):
            if j % (self.canvas_size / 10) == 0:
                ...
                if k != 0:
                    self.displayer.create_line(-2 + y_position, k + self.canvas_size // 2, 2 + y_position, k + self.canvas_size // 2,
                                     width=0.25, fill='black')

                    self.displayer.create_text(15 + y_position, k + self.canvas_size // 2,
    ...
    def add_function(self, f, color="black"):
        ...
        for x in range(self.canvas_size + 1):
            ...
            self.displayer.create_line(previous_point, point, fill=color)
            previous_point = point

    def add_point(self, x, y, color="black"):
        self.displayer.create_oval(
            x - 1,
    ...
    def on_click(self):
        ...
        self.rescale(f, x_min, x_max, y_min, y_max)
